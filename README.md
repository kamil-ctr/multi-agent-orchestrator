...
- 📡 **Real-time SSE streaming** — the frontend watches each agent go `waiting → running → done/error` live via Server-Sent Events, with every agent's response rendered token-by-token as it's generated (toggle via `STREAMING_ENABLED`), not a spinner and a single final payload
...
flowchart LR
    ...
    FE -->|POST /api/conversations/id/messages| API[FastAPI]
    API -->|query_id| FE
    ...
    API --> Pipeline
    Pipeline -->|agent_start / agent_token / agent_done / agent_error / synthesis_done| SSE
    SSE --> FE
    FE --> U
```
...
|---|---|---|---|---|
| `POST` | `/api/query` | Legacy shortcut: starts a pipeline run as a background task, creating a single-message conversation under the hood | `{prompt: str, image_base64?: str, image_mime?: str, file_context?: str, file_name?: str, use_cache?: bool, enabled_agents?: string[], semantic_cache_enabled?: bool, semantic_cache_threshold?: float}` | `{query_id: str, conversation_id: int}` |
| `GET` | `/api/query/{id}/stream` | SSE stream of pipeline lifecycle events | — | `event: agent_start\|agent_token\|agent_done\|agent_error\|synthesis_done\|fatal_error`, each with a JSON `data:` payload. `agent_token` carries `{agent, token}` for each text delta as it's generated |
| `POST` | `/api/conversations` | Creates a new (empty) conversation | `{title?: str}` | `{id, title, created_at, updated_at, message_count}` |
| `GET` | `/api/conversations` | Paginated conversation list, newest-updated first | query params: `page, page_size, search?` | `{page, page_size, total, total_pages, items: [...]}` |
| `GET` | `/api/conversations/{id}` | Full message history for one conversation | — | `{id, title, created_at, updated_at, message_count, messages: [{id, role, content, agent_responses, created_at}]}` |
| `POST` | `/api/conversations/{id}/messages` | Primary query entrypoint — sends a message, passing recent conversation history as context | same body as `/api/query` | `{query_id: str, conversation_id: int}` |
| `PATCH` | `/api/conversations/{id}` | Renames a conversation | `{title: str}` | Updated conversation metadata |
| `DELETE` | `/api/conversations/{id}` | Deletes a conversation and its messages | — | `{status: "deleted"}` |
...
│   │   ├── cache.py                # SQLite exact + semantic prompt cache, TTL-based
│   │   ├── embeddings.py             # Cohere embed API + cosine similarity (no local ML runtime)
│   │   ├── history.py                 # SQLite query history + leaderboard aggregation
│   │   ├── conversations.py            # SQLite conversations/messages, context building + trimming
│   │   ├── jobs.py                      # shared in-memory SSE job store (query.py + conversations.py)
...
│   │   ├── dispatch.py            # Stage 2 — parallel dispatch (+ per-agent image routing/streaming)
...
│   ├── api/                      # FastAPI routers: query, conversations, upload, history, leaderboard, agents, export
...
└── frontend/
    └── src/
        ...
        ├── components/            # ChatInput, AgentCard, ResultsPanel, ComparisonTable, ConversationSidebar, ...
